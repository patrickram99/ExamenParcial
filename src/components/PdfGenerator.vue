<template>
  <button
    ...
    class="flex items-center justify-center px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 disabled:bg-blue-400"
  >
    <template v-if="isGenerating">
      <loader-2 :size="18" class="mr-2 animate-spin" />
      Generando PDF...
    </template>
    <template v-else>
      ...
      try {
        ...
        if (generalInfoSection) {
          generalInfoSection.querySelectorAll("p").forEach((item) => {
            addParagraph(item.textContent?.replace("(Editable)", "").trim() || "")
          })
        }
        y += 5
        ...
        if (unitsSection) {
          ...
          unitTables.forEach((table) => {
            ...
            const unitNumber = unitHeader?.querySelector("th:nth-child(1)")?.textContent?.replace("UNIDAD", "").trim() || ""
            const unitTitle = unitHeader?.querySelector("th:nth-child(2)")?.textContent?.replace("(Editable)", "").trim() || ""
            const rows = Array.from(table.querySelectorAll("tbody tr"))
            const firstRow = rows[0]
            const period = firstRow?.querySelector("td:nth-child(1)")?.textContent?.replace("(Editable)", "").trim() || ""
            const expectedResult = firstRow?.querySelector("td:nth-child(2)")?.textContent?.replace("(Editable)", "").trim() || ""
            const weeklyContent = []
            if (rows[0]) {
              weeklyContent.push({
                week: rows[0]?.querySelector("td:nth-child(3)")?.textContent?.trim() || "",
                content: rows[0]?.querySelector("td:nth-child(4)")?.textContent?.replace("(Editable)", "").trim() || "",
              })
            }
            for (let i = 1; i < rows.length - 2; i++) {
              const week = rows[i]?.querySelector("td:nth-child(1)")?.textContent?.trim() || ""
              const content = rows[i]?.querySelector("td:nth-child(2)")?.textContent?.replace("(Editable)", "").trim() || ""
              if (week && content) weeklyContent.push({ week, content })
            }
            const methodology = rows[rows.length - 2]?.textContent?.replace("Metodología", "").replace("(Editable)", "").trim() || ""
            const sourcesList = []
            rows[rows.length - 1]?.querySelectorAll("li")?.forEach((item) => {
            ...
            autoTable(pdf, {
              ...
              body: [
                [
                  { content: period, rowSpan: weeklyContent.length, styles: { valign: "top" } },
                  { content: expectedResult, rowSpan: weeklyContent.length, styles: { valign: "top" } },
                  { content: weeklyContent[0]?.week || "", styles: { halign: "center" } },
                  { content: weeklyContent[0]?.content || "" },
                ],
                ...weeklyContent
                  .slice(1)
                  .map((item) => [{ content: item.week, styles: { halign: "center" } }, { content: item.content }]),
        ...
        if (evaluationSection) {
          ...
          if (evaluationTable) {
            ...
            const headerRow = Array.from(headerCells).map((cell) => ({
              ...
              styles: { halign: "center", fillColor: [229, 229, 229], textColor: [0,0,0] },
            }))
            const bodyRows = evaluationTable.querySelectorAll("tbody tr")
            bodyRows.forEach((row, index) => {
              const cells = row.querySelectorAll("td")
              const rowData = Array.from(cells).map((cell) => ({
                content: cell.textContent?.replace("(Editable)", "").trim() || "",
                styles: { fillColor: index % 2 === 0 ? [229, 229, 229] : null, textColor: [0,0,0] },
              }))
        ...
        if (signatureSection) {
          addParagraph(signatureSection.querySelector("p:nth-child(1)")?.textContent?.replace("(Editable)", "").trim() || "")
          addParagraph(signatureSection.querySelector("p:nth-child(2)")?.textContent?.replace("Subir firma", "").trim() || "")
          
          // Try to add the signature image if it exists
          ...
          if (signatureImg && signatureImg.src) {
            try {
              ...
              if (signatureImg.src.startsWith('data:image')) {
                pdf.addImage(signatureImg.src, 'JPEG', margin, y, 50, 20)
              } else {
